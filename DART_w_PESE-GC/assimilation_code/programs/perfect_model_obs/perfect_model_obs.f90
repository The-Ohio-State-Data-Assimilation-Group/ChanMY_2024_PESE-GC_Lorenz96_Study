...
AdvanceTime: do
   ...
   write(msgstring, '(A)') 'Obs values computed'
   call trace_message(msgstring, 'perfect_model_obs:', -1)
   ...
   if(my_task_id() == 0) then
      ...
      do i = 1, fwd_op_ens_handle%num_vars
         ...
         if( qc_ens_handle%vars(i, 1) == 0 ) then
            ...
            !!! JOSEPH DISABLED LINES !!! ! Get the information for generating error sample for this observation
            !!! JOSEPH DISABLED LINES !!! if(use_algorithm_info_mod) then
            !!! JOSEPH DISABLED LINES !!!    call obs_error_info(obs_def, error_variance, &
            !!! JOSEPH DISABLED LINES !!!       bounded_below, bounded_above, lower_bound, upper_bound)
            !!! JOSEPH DISABLED LINES !!! else
            !!! JOSEPH DISABLED LINES !!!    ! Default is unbounded with standard error_variance
            !!! JOSEPH DISABLED LINES !!!    error_variance = get_obs_def_error_variance(obs_def)
            !!! JOSEPH DISABLED LINES !!!    bounded_below = .false. ; bounded_above = .false.
            !!! JOSEPH DISABLED LINES !!!    lower_bound = 0.0_r8;     upper_bound = 0.0_r8
            !!! JOSEPH DISABLED LINES !!! endif
