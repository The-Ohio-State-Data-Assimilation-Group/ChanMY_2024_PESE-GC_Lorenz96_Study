...
use pese_mod,              only : init_pese_info, apply_pese, pese_type, &
...
use algorithm_info_mod, only : probit_dist_info, algorithm_info_mod_init
...
! Allocate variables relating to Probit-space Ensemble Size Expansion
logical              :: use_pese             = .false.
integer              :: ens_size_expanded    = -1        ! Size of PESE-expanded ensemble
integer              :: ens_size_virtual     = -1        ! Number of PESE-generated members
integer              :: ens_size_dynamic     = -1        ! Number of members created by forecast model
logical              :: print_pese_debug     =.false.    ! Should I print out debug messages for PESE?
!!! DELETE !!! integer              :: pese_marginal        = -1        ! Specifies which marginals to use for PESE (1: Gaussian)
!!! DELETE !!! real                 :: pese_avg1, pese_avg2, pese_std1, pese_std2
...
! initialize algorithm_info_mod
call algorithm_info_mod_init()

! If using PESE, do some variable initializations
PREPARE_PESE: if (use_pese) then

   write(msgstring, *) 'Probit-space Ensemble Size Expansion (PESE) will be used.'

   ! ens_size now refers to the expanded ensemble size
   ens_size_dynamic = ens_size
   ens_size = ens_size_expanded
   ens_size_virtual = ens_size_expanded - ens_size_dynamic

   ! Current PESE implementation requires ens_size_virtual > ens_size_dynamic
   ! Check if this requirement holds
   if(ens_size_virtual <= ens_size_dynamic) then
      write(msgstring, *) 'Specified ens_size_expanded in namelist is too small.'
      write(msgstring, *) 'Minimum ens_size_expanded value is 2 * ens_size + 1'
      write(msgstring, *) 'Current ens_size_expanded value:', ens_size
      write(msgstring, *) 'Minimum ens_size_expanded value:', ens_size_dynamic*2+1
      call error_handler(E_ERR,'filter_main', msgstring, source)
   endif

   ! Initialize structured type needed to run PESE
   call init_pese_info( ens_size, ens_size_dynamic )
...
! If PESE is used, overwrite IO flags to prevent attempts to read/write 
! virtual members.
if (use_pese) call pese_modify_all_file_info_io_flags(                        &
      ens_size_dynamic, ens_size, ENS_MEM_START, file_info_input,             &
      file_info_forecast, file_info_preassim, file_info_postassim,            &
      file_info_analysis, file_info_output                                    &
)
...
!!! DELETE !!! pese_avg1 = sum( state_ens_handle%copies(1:ens_size_dynamic,1) )/ens_size_dynamic
!!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size_dynamic,1)-pese_avg1)**2 )/(ens_size_dynamic-1)
!!! DELETE !!! pese_std1 = sqrt( pese_std1)
!!! DELETE !!! write(*,*) ' after_read_state:  pre-PESE avg and std', pese_avg1, pese_std1

! Apply PESE to generate virtual ensemble members
if ( use_pese ) call apply_pese( state_ens_handle, print_pese_debug )

!!! DELETE !!! pese_avg2 = sum( state_ens_handle%copies(1:ens_size,1) )/ens_size
!!! DELETE !!! pese_std2 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg2)**2 )/(ens_size-1.)
!!! DELETE !!! pese_std2 = sqrt( pese_std2)
!!! DELETE !!! write(*,*) ' after_read_state: post-PESE avg and std', pese_avg2, pese_std2
...
AdvanceTime : do
   ...
   if (curr_ens_time /= next_ens_time) then
      ...
      ! When PESE is involved, only integrate dynamic members and then do resampling
      if ( use_pese ) then

         !!! DELETE !!! pese_avg1 = sum( state_ens_handle%copies(1:ens_size_dynamic,1) )/ens_size_dynamic
         !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size_dynamic,1)-pese_avg1)**2 )/(ens_size_dynamic-1)
         !!! DELETE !!! pese_std1 = sqrt( pese_std1)
         !!! DELETE !!! write(*,*) '  b4 advance_state: avg and std', pese_avg1, pese_std1 

         call advance_state(state_ens_handle, ens_size_dynamic, next_ens_time, async, &
         ...
         !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size_dynamic,1)-pese_avg1)**2 )/(ens_size_dynamic-1)
         !!! DELETE !!! pese_std1 = sqrt( pese_std1)
         !!! DELETE !!! write(*,*) '  pre-PESE avg and std', pese_avg1, pese_std1


         call apply_pese( state_ens_handle, print_pese_debug )
         call all_copies_to_all_vars(state_ens_handle)

         !!! DELETE !!! pese_avg2 = sum( state_ens_handle%copies(1:ens_size,1) )/ens_size
         !!! DELETE !!! pese_std2 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg2)**2 )/(ens_size-1.)
         !!! DELETE !!! pese_std2 = sqrt( pese_std2)
         !!! DELETE !!! write(*,*) ' post-PESE avg and std', pese_avg2, pese_std2
      
      ! If PESE is not used, advance the entire ensemble
      else

         !!! DELETE !!! pese_avg1 = sum( state_ens_handle%copies(1:ens_size,1) )/ens_size
         !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 )/(ens_size-1)
         !!! DELETE !!! pese_std1 = sqrt( pese_std1)
         !!! DELETE !!! write(*,*) '  b4 advance_state: avg and std', pese_avg1, pese_std1
         ...
         !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 )/(ens_size-1)
         !!! DELETE !!! pese_std1 = sqrt( pese_std1)
         !!! DELETE !!! write(*,*) '  aft advance_state: avg and std', pese_avg1, pese_std1
      ...
      !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 )/(ens_size-1)
      !!! DELETE !!! pese_std1 = sqrt( pese_std1)
      !!! DELETE !!! write(*,*) '   b4 all_vars_to_all_copies: avg and std', pese_avg1, pese_std1
      ...
      !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 )/(ens_size-1)
      !!! DELETE !!! pese_std1 = sqrt( pese_std1)
      !!! DELETE !!! write(*,*) '  aft all_vars_to_all_copies: avg and std', pese_avg1, pese_std1
      ...
      !!! DELETE !!! pese_std1 = sum(  (state_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 )/(ens_size-1)
      !!! DELETE !!! pese_std1 = sqrt( pese_std1)
      !!! DELETE !!! write(*,*) '  aft compute_copy_mean_sd: avg and std', pese_avg1, pese_std1
   ...
   !!! DELETE !!! pese_avg1 = sum( obs_fwd_op_ens_handle%copies(1:ens_size,1) ) / ens_size
   !!! DELETE !!! pese_std1 = sum( (obs_fwd_op_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 ) / (ens_size-1.)
   !!! DELETE !!! pese_std1 = sqrt(pese_std1)
   !!! DELETE !!! write(*,*) ' yf avg and std', pese_avg1, pese_std1

   !!! DELETE !!! pese_avg1 = sum( state_ens_handle%copies(1:ens_size,1) ) / ens_size
   !!! DELETE !!! pese_std1 = sum( (state_ens_handle%copies(1:ens_size,1)-pese_avg1)**2 ) / (ens_size-1.)
   !!! DELETE !!! pese_std1 = sqrt(pese_std1)
   !!! DELETE !!! write(*,*) ' xf avg and std', pese_avg1, pese_std1
