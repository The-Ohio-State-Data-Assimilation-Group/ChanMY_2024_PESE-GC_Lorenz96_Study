...
! Check if PESE is being used
! ---------------------------
ens_size_dynamic = 0
! Count number of members that will be read
do i = 1, ens_size
   if (query_read_copy(file_handle%stage_metadata, i)) then
      ens_size_dynamic = ens_size_dynamic + 1
   end if
end do
! Determine if PESE is used
if (ens_size_dynamic < ens_size ) then
   use_pese = .true.
else
   use_pese = .false.
end if

! If PESE is used, check if dynamic members exist in file
if ( use_pese ) then
   call check_singlefile_member_info(my_ncid, fname, ens_size_dynamic, do_perturb)

! If PESE is not used, check if all ensemble members exist in file
else
